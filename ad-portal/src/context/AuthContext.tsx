...
function loadUser(): PortalUser | null {
  const raw = localStorage.getItem('adPortalUser');
  if (!raw) return null;
  try {
...
export function AuthProvider({ children }: { children: ReactNode }) {
  const [token, setToken] = useState<string | null>(() => localStorage.getItem('adPortalToken'));
  ...
  const value = useMemo<AuthContextValue>(() => {
    ...
    return {
      ...
      login(nextToken, nextUser) {
        localStorage.setItem('adPortalToken', nextToken);
        localStorage.setItem('adPortalUser', JSON.stringify(nextUser));
        setToken(nextToken);
        setUser(nextUser);
      },
      logout() {
        localStorage.removeItem('adPortalToken');
        localStorage.removeItem('adPortalUser');
        setToken(null);
        setUser(null);
      },
