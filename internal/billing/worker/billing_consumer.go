...
func runBillingConsumer(ctx context.Context, db *gorm.DB, rdb *platformredis.RedisClient, logger *slog.Logger) {
	...
	for {
		...
		ids, err := processBillingBatch(ctx, db, rdb, events, campaigns, subs, rates, budgetFlags, msgs, logger)
		if err != nil {
...
func processBillingBatch(
	...
	_ *billingInfra.BillingEventRepository,
	...
	for _, msg := range msgs {
		...
		if err != nil {
			logger.Warn("billing consumer: skip message", "id", msg.ID, "error", err)
	...
	if len(built) > 0 {
		if err := db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
			return billingInfra.NewBillingEventRepository(tx).CreateBatch(ctx, built)
		}); err != nil {
			return nil, err
		}
	}
